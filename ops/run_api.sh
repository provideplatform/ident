...
if [[ -z "${AUTH0_INTEGRATION_ENABLED}" ]]; then
  AUTH0_INTEGRATION_ENABLED=false # sandbox creds
fi
...
if [[ -z "${IDENTITYMIND_API_TOKEN}" ]]; then
  IDENTITYMIND_API_TOKEN=fc45e5e06411423ed8e0bc80849e40b52a538d03
fi

if [[ -z "${VOUCHED_API_TOKEN}" ]]; then
  VOUCHED_API_TOKEN='6Hglh6svY>V@!Lsj>+dNM^6eqh76zz'
fi

# SMTP Server settings for sending emails (such as welcome email).
...
IDENTITYMIND_API_USER=${IDENTITYMIND_API_USER} \
